...
class Metrics:
    ...
    def get_arguments_names(self, givenTool):
        ...
        for arg in givenTool["arguments"]:
            if "argument_name" in arg:
                ...
                arguments_names.append(arg["name"])
    ...
    def precision(self, predictedList, requiredList):
        ...
        if isinstance(predictedList, list) == False:
            predictedList = [predictedList]
        if isinstance(requiredList, list) == False:
    ...
    def hallucination_rate(self, given_json, predicted_json):
        ...
        predicted_args = []
        for tool in predicted_json:
            for arg in tool["arguments"]:
                if "argument_name" in arg:
                    predicted_args.append(tool["tool_name"]+arg["argument_name"])
                else:
                    predicted_args.append(tool["tool_name"]+arg["name"])

        given_args = []
        for tool in given_json:
            for arg in tool["arguments"]:
                if "argument_name" in arg:
                    given_args.append(tool["tool_name"]+arg["argument_name"])
                else:
                    given_args.append(tool["tool_name"]+arg["name"])
    ...
    def f1_score_overall(self, predictedJson, requiredJson):
        ...
        for correct_tool_name in correct_tool_names:
            ...
            for correct_arg_name in correct_arg_names:
                ...
                if found_arg != {}:
                    if "argument_value" in found_arg:
                        predicted_values = found_arg["argument_value"]
                    else:
                        predicted_values = found_arg["value"]
                ...
                if found_arg != {}:
                    if "argument_value" in found_arg:
                        ...
                        required_values = found_arg["value"]
